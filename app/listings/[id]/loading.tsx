  ...
  return (
    <main className="min-h-screen bg-[#F8F5F2] px-4 py-8">
      <div className="mx-auto max-w-6xl">
        <div className="aspect-[4/3] animate-pulse rounded-3xl bg-stone-200" />
          ...
          <section>
            ...
            <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3">
              {Array.from({ length: 6 }).map((_, index) => (
                <div key={index} className="h-10 animate-pulse rounded-full bg-stone-200" />
              ))}
            </div>
          </section>
          <aside className="h-64 animate-pulse rounded-3xl bg-stone-200" />
        </div>
      </div>
    </main>
  )
