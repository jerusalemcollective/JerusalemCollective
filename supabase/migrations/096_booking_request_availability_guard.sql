-- 096_booking_request_availability_guard.sql
--
-- Stop a guest requesting to book dates that are already blocked. Until now
-- create_listing_enquiry() never checked availability, so a "Request to book"
-- could be created over an existing block; availability was only enforced on the
-- instant Book-now path and by the confirmed-booking exclusion constraint.
--
-- This redefines create_listing_enquiry() (last defined in 065) adding the same
-- overlap test the Book-now API uses: an existing unavailable range overlaps the
-- requested stay when  start_date < check_out AND end_date > check_in
-- (ranges are stored start_date = check_in, end_date = check_out). The check only
-- runs when both dates are supplied, so open-ended enquiries are unaffected.
...
begin
  ...
  -- Availability guard: block requests over dates that are already taken.
  if check_in_date is not null and check_out_date is not null then
    if exists (
      select 1
      from public.listing_unavailable_ranges r
      where r.listing_id = target_listing_id
        and r.start_date < check_out_date
        and r.end_date > check_in_date
    ) then
